...
// Função para obter o estado do contador do Netlify Blobs.
async function getCounterState() {
    try {
        const state = await get("dailyCounter", { type: "json" });
        return state || { count: 0, lastReset: '1970-01-01' };
    } catch (e) {
        console.error("Erro ao obter o contador do Blob:", e);
        // Retorna um estado inicial seguro em caso de falha.
        return { count: 0, lastReset: '1970-01-01' };
    }
}

// Função para atualizar o estado do contador no Netlify Blobs.
async function updateCounterState(newState) {
    try {
        await set("dailyCounter", JSON.stringify(newState), { type: "json" });
    } catch (e) {
        console.error("Erro ao salvar o contador no Blob:", e);
        throw e;
    }
}
